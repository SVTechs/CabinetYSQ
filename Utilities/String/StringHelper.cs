...
namespace Utilities.String
{
    ...
    public class StringHelper
    {
        public static string GetHash(string origin)
        {
            ...
            string hashBase = Md5Encode.Encode(origin.Substring(0, origin.Length / 2), false)
                              + Md5Encode.Encode(origin.Substring(origin.Length / 2), false);
            string factor = Md5Encode.Encode(hashBase.Substring(0, 16), false).Substring(0, 16);
            string output = JAesEncrypt.Encrypt(hashBase, factor);
            return Md5Encode.Encode(output, false).Substring(0, 3);
        }

        public static string FormatToLength(string input, int targetLen)
        {
            if (string.IsNullOrEmpty(input)) return "";
            while (input.Length < targetLen) input = "0" + input;
            return input;
        }

        public static string AddSpace(string origin, int interval)
        {
            if (string.IsNullOrEmpty(origin) || interval <= 0) return "";
            StringBuilder sb = new StringBuilder();
            for (int i = 0; i < origin.Length; i+= interval)
            {
                sb.Append(origin.Substring(i, interval) + " ");
            }
            return sb.ToString();
        }

        public static string ToAscBytes(string input)
        {
            string opBytes = "";
            for (int i = 0; i < input.Length; i++)
            {
                opBytes += CharToAsc(input[i].ToString());
            }
            return opBytes;
        }

        public static string ToTextDate(DateTime origin, int formatType)
        {
            switch (formatType)
            {
                case 5:
                    return origin.Year.ToString("X3") + origin.Month.ToString("X") + origin.Day.ToString("X");
                case 7:
                    return origin.Year.ToString("X3") + origin.Month.ToString("D2") + origin.Day.ToString("D2");
            }
            return origin.ToString("YYYYMMDD");
        }

        public static DateTime ToDate(string origin, int formatType)
        {
            int year, month, day;
            switch (formatType)
            {
                case 5:
                    year = int.Parse(origin.Substring(0, 3), NumberStyles.HexNumber);
                    month = int.Parse(origin.Substring(3, 1), NumberStyles.HexNumber);
                    day = int.Parse(origin.Substring(4, 1), NumberStyles.HexNumber);
                    return DateTime.Parse(year + "-" + month.ToString("D2") + "-" + day.ToString("D2"));
                case 7:
                    year = int.Parse(origin.Substring(0, 3), NumberStyles.HexNumber);
                    return DateTime.Parse(year + "-" + origin.Substring(3, 2) + "-" + origin.Substring(5, 2));
            }
        ...
        private static string CharToAsc(string chr)
        {
            ...
            if (IsTwoBytesChar(chr))
            {
                // 双字节码为高位乘256，再加低位
                // 该为无符号码，再减65536
                string resChar = (codeBytes[0] * 256 + codeBytes[1] - 65536).ToString("X4");
                if (resChar.Length > 4) resChar = resChar.Substring(4);
                return resChar;
            }
            else
            {
                return codeBytes[0].ToString("X2");
            }
        }
        ...
        public static string GetSpellCode_Full(string str)
        {
