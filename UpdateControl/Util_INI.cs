...
namespace UpdateControl
{
    public class INIHelper
    {
        ...
        public static void WriteEx(string Section, string Key, string Value, string path)
        {
            string EncValue = AesEncryption.Encrypt(Value, "update TInfo set", "1234567812345678");
            WritePrivateProfileString(Section, Key, EncValue, path);
        }
        ...
        public static string ReadEx(string Section, string Key, string path)
        {
            StringBuilder temp = new StringBuilder(255);
            int i = GetPrivateProfileString(Section, Key, "", temp, 255, path);
            string DecValue = AesEncryption.Decrypt(temp.ToString(), "update TInfo set", "1234567812345678");
            return DecValue;
        }
