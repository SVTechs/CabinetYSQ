...
{
    public class CabinetDeviceRx5 : ICabinetDevice //迎水桥工具柜
    {
        ...
        private static int _doorKeepTime = 120;
        ...
        private void LockScheduler()
        {
            while (true)
            {
                ...
                try
                {
                    ...
                    {
                        if ((DateTime.Now - _lastDoorUnlock).TotalSeconds > _doorKeepTime && _lastDoorUnlock != DateTime.MinValue)
                        {
                            OpenLedGreen(false);
                            OpenLedYellow(false);
                            OpenLedRed(true);
        ...
        public int OpenLedGreen(bool status)
        {
            ...
            {
                if (!_canBusGroup[0].CanbusCtrl.Connected) return -1;
                _canBusGroup[0].CanbusCtrl.WriteSingleCoil(1, (ushort)_ledGreenCtrlPoint, status);
                return 1;
            }
            catch (Exception)
            {
