...
{
    public class CabinetDeviceRx6 : ICabinetDevice //工具车
    {
        ...
        private static int _doorKeepTime = 10; //原为120
        private static int _drawerLockTime = 30;
        private static int _buttonPress = 1;
        ...
        private static int _doorOpenNum, _lastDoorOpenNum;
        ...
        //延时信息
        private static DateTime _lastDoorUnlock, _lastDoorLock, _lastDoorOpen, _lastButtonPress;
        ...
        private static bool _doorCanOpen = false;
        ...
        private void ToolStatusMonitor()
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    {
                        //门控
                        if (curDoorStatus == 0 && doorStatus == 1 && !_doorCanOpen)
                        {
        ...
        private void LockScheduler()
        {
            while (true)
            {
                ...
                try
                {
                    ...
                    lock (DelayLock)
                    {
                        if ((DateTime.Now - _lastDoorOpen).TotalSeconds >= _doorKeepTime)
                        {
                            _doorCanOpen = false;
                        }
                    }
                    if (_doorOpenNum > 0)
                    {
                        //if (_doorOpenNum != _lastDoorOpenNum && (DateTime.Now - _lastButtonPress).TotalSeconds >= _buttonPress) return;
                        UnlockDoor(true);
                    }
                    //抽屉锁定控制
                    //for (int i = 0; i < _drawerCount; i++)
        ...
        private int IsDoorClosed()
        {
            ...
            try
            {
                ...
                if (statusBuffer[0] && statusBuffer[1] && statusBuffer[2] && statusBuffer[3] && statusBuffer[4] && statusBuffer[5]) return 1;
                return 0;
            }
            catch (Exception)
            {
        ...
        public int UnlockDrawer(bool[] status, bool updateSignal = true)
        {
            ...
            try
            {
                ...
                //解锁抽屉
                _canBusGroup[0].CanbusCtrl.WriteMultipleCoils(1, 0, status);
                return 1;
            }
            catch (Exception)
            {
        ...
        public int UnlockDrawer(int drawerNo, bool status, bool updateSignal = true)
        {
            ...
            try
            {
                ...
                //解锁抽屉
                //_canBusGroup[0].CanbusCtrl.WriteSingleCoil(1, 8 + drawerNo, status);
                return 1;
            }
            catch (Exception)
            {
        ...
        public int UnlockDoor(bool status, bool updateSignal = true)
        {
            ...
            if (status)
            {
                if (IsDoorClosed() != 1) return -2;
                if (_doorOpenNum <= 0) return -2;
                if (!_doorCanOpen) return -2;
            }
            try
            {
                if (!_canBusGroup[0].CanbusCtrl.Connected) return -1;
                if (updateSignal)
                {
                    lock (DelayLock)
                    {
                        ...
                        else
                        {
                            ...
                            _lastDoorOpen = DateTime.MinValue;
                        }
                    }
                }
                if (status) _isDoorTimeoutClosed = false;
                bool[] b = {false, false, false, false, false, false};
                if (status) b[_doorOpenNum - 8] = true;
                //柜门操作
                for (int i = 0; i < 4; i++)
                {
                    if (status)
                    {
                        _canBusGroup[0].CanbusCtrl.WriteMultipleCoils(1, 0, b);
                        _lastDoorUnlock = DateTime.Now;
                        _lastDoorOpen = DateTime.Now;
                        _lastDoorOpenNum = _doorOpenNum;
                    }
                    else
                    {
                        _canBusGroup[0].CanbusCtrl.WriteMultipleCoils(1, 0, b);
        ...
        public int OpenLedGreen(bool status)
        {
            ...
            {
                if (!_canBusGroup[0].CanbusCtrl.Connected) return -1;
                //_canBusGroup[0].CanbusCtrl.WriteSingleCoil(1, _ledGreenCtrlPoint, status);
                return 1;
            }
            catch (Exception)
            {
        ...
        private static bool[] IsToolExist()
        {
            ...
            try
            {
                ...
                //fullResult[0] = statusList[1];
                //fullResult[1] = statusList[0];
                //fullResult[2] = statusList[3];
                //fullResult[3] = statusList[4];
                //fullResult[4] = statusList[2];
                //映射修正
                foreach (DictionaryEntry de in BrokenCheckerMapping)
                {
                    int origin = (int)de.Key;
                    int dest = (int)de.Value;
                    fullResult[dest] = statusList[origin];
