...
{
    public class CabinetDeviceRx2 : ICabinetDevice
    {
        ...
        private static int _ledGreenCtrlPoint = -1;
        ...
        private static bool FixConnection(bool fastCheck = false)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < _toolCheckerGroup.Count; i++)
                {
                    if (_toolCheckerGroup[i].ControllerType == 0)
                    {
                        ...
                        {
                            TcpClientEx client = new TcpClientEx(_toolCheckerGroup[i].ConnAddr, _toolCheckerGroup[i].NetPort, 8000);
            ...
            try
            {
                ...
                for (int i = 0; i < _toolLedOperatorGroup.Count; i++)
                {
                    if (_toolLedOperatorGroup[i].ControllerType == 0)
                    {
                        ...
                        {
                            TcpClientEx client = new TcpClientEx(_toolLedOperatorGroup[i].ConnAddr, _toolLedOperatorGroup[i].NetPort, 8000);
            ...
            try
            {
                ...
                for (int i = 0; i < _toolCheckerGroup.Count; i++)
                {
                    if (_toolCheckerGroup[i].ControllerType == 0)
                    {
                        if (_cabinetCheckerIp == _toolCheckerGroup[i].ConnAddr && _cabinetCheckerPort == _toolCheckerGroup[i].NetPort)
                        {
                ...
                for (int i = 0; i < _toolLedOperatorGroup.Count; i++)
                {
                    if (_toolLedOperatorGroup[i].ControllerType == 0)
                    {
                        if (_cabinetOperatorIp == _toolLedOperatorGroup[i].ConnAddr && _cabinetOperatorPort == _toolLedOperatorGroup[i].NetPort)
                        {
        ...
        public int UnlockDoor(bool status, bool updateSignal = true)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    if (status)
                    {
                        _cabinetOperatorMaster.WriteMultipleCoils(1, (ushort) (4 + i * 2),
                            new[] {false, true});
                    }
                    else
                    {
                        _cabinetOperatorMaster.WriteSingleCoil(1, (ushort)(5 + i * 2), false);
                    }
                }
                return 1;
            }
        ...
        public int LockDoor(bool status, bool updateSignal = true)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    {
                        _cabinetOperatorMaster.WriteSingleCoil(1, (ushort)(4 + i * 2), false);
                    }
                }
                return 1;
            }
