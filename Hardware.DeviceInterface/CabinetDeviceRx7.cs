...
{
    public class CabinetDeviceRx7 : ICabinetDevice //样柜
    {
        ...
        private static int _operateTime = 45;
        ...
        private static int _lightCtrlPoint = 11;
        private static int _ledGreenCtrlPoint = 13;
        private static int _ledYellowCtrlPoint = 12;
        ...
        //延时信息
        private static DateTime _lastDoorUnlock, _lastDoorLock, _loginTime;
        ...
        private static bool _canOperate = false;
        ...
        private void ToolStatusMonitor()
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    {
                        //门控
                        if (curDoorStatus == 0 && doorStatus == 1 && !_canOperate)
                        {
        ...
        private void LockScheduler()
        {
            while (true)
            {
                ...
                int drawerCloseStatus = IsDrawerClosed();
                ...
                try
                {
                    ...
                    lock (DelayLock)
                    {
                        if ((DateTime.Now - _loginTime).TotalSeconds >= _operateTime)
                        {
                            _canOperate = false;
                            if (doorCloseStatus != 1 || drawerCloseStatus != 1)
                            {
                                OpenLedYellow(true);
                            }
                        }
                    ...
                    for (int i = 0; i < _drawerCount; i++)
                    {
                        ...
                        {
                            //延时期过后停止解锁过程
                            if ((DateTime.Now - _lastDrawerUnlock[i]).TotalSeconds >= _operateTime)
                            {
                                result = true;
                            }
                        }
        ...
        private int IsDrawerClosed()
        {
            ...
            try
            {
                ...
                Hashtable infoBuffer = _canBusGroup[0].CanbusCtrl.GetStatusBuffer();
                bool[] statusBuffer = (bool[])infoBuffer[1];
                if (statusBuffer[1] && statusBuffer[3] && statusBuffer[5]) return 1;
                return 0;
            }
            catch (Exception)
            {
        ...
        private int IsButtonPressed()
        {
            ...
            try
            {
                ...
                Hashtable infoBuffer = _canBusGroup[0].CanbusCtrl.GetStatusBuffer();
                bool[] statusBuffer = (bool[])infoBuffer[1];
                if (statusBuffer[7] || statusBuffer[8] || statusBuffer[9]) return 1;
                return 0;
            }
            catch (Exception)
            {
        ...
        public int UnlockDrawer(int drawerNo, bool status, bool updateSignal = true)
        {
            ...
            if (status)
            {
                if (IsDrawerClosed() != 1) return -2;
                if (IsButtonPressed() <= 0) return -2;
                if (!_canOperate) return -2;
            }
            try
            {
                if (!_canBusGroup[0].CanbusCtrl.Connected) return -1;
                ...
                //解锁抽屉
                _canBusGroup[0].CanbusCtrl.WriteSingleCoil(1, 0 + drawerNo, status);
                return 1;
            }
            catch (Exception)
            {
        ...
        public int UnlockDoor(bool status, bool updateSignal = true)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    if (status)
                    {
                        _canBusGroup[0].CanbusCtrl.WriteMultipleCoils(1, 6, new[] { true });
                    }
                    else
                    {
                        _canBusGroup[0].CanbusCtrl.WriteMultipleCoils(1, 6, new[] { false });
                    }
                }
